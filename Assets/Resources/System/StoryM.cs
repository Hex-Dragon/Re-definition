...
public class StoryM : MonoBehaviour {
    ...
    IEnumerator Stage3() {
        ...
        arrowMax = 0;
        Spawn(Spawner.EnemyType.Mover, 1);
        yield return StartCoroutine(WaitUntilClear(0, 6f, "这里一个骰子都没有！切题呢？作者到底咋想的？", "Not a single dice here! What's the point? What‘s wrong with this author?"));
        stageCompleted = true;
    }
    IEnumerator Stage4() {
        arrowMax = 0;
        ...
        yield return StartCoroutine(WaitUntilPickDice(5f, "看到右边掉下来的东西了么？把它捡起来摇一摇！", "You see the falling stuff on the right? Pick it up and try shaking it!"));
        yield return new WaitForSeconds(2f);
        yield return StartCoroutine(WaitForStoryText("你看，完美！", "See? Perfect!"));
        stageCompleted = true;
    }
    IEnumerator Stage5() {
