...
public class TextPic : Text, IPointerClickHandler, IPointerExitHandler, IPointerEnterHandler, ISelectHandler {
	...
	private static List<IconName> _iconList;
	private static List<IconName> iconList {
		get {
			if (_iconList == null) {
				_iconList = new List<IconName>();
				foreach (Sprite sprite in Resources.LoadAll<Sprite>("Controls/TextPic")) {
					_iconList.Add(new IconName { name = sprite.name, sprite = sprite, offset = Vector2.zero, scale = Vector2.one });
				}
				Debug.Log("加载了 " + _iconList.Count + " 个 TextPic 图标");
			}
			return _iconList;
		}
	}
	...
	[SerializeField]
	private HrefClickEvent m_OnHrefClick = new();
	...
	/// Image Pool
	/// </summary>
	private readonly List<Image> m_ImagesPool = new();
	private readonly List<GameObject> culled_ImagesPool = new();

	// Used for check for culling images
	private bool clearImages = false;

	// Lock to ensure images get culled properly
	private UnityEngine.Object thisLock = new();

	/// <summary>
	/// Vertex Index
	/// </summary>
	private readonly List<int> m_ImagesVertexIndex = new();

	/// <summary>
	/// Regular expression to replace 
	/// </summary>
	private static readonly Regex s_Regex =
		new(@"<quad name=(.+?) size=(\d*\.?\d+%?) width=(\d*\.?\d+%?) />", RegexOptions.Singleline);
	...
	/// Hyperlink List
	/// </summary>
	private readonly List<HrefInfo> m_HrefInfos = new();

	/// <summary>
	/// Text Builder
	/// </summary>
	private static readonly StringBuilder s_TextBuilder = new();
	...
	private static readonly Regex remove_Regex =
		new(@"<b>|</b>|<i>|</i>|<size=.*?>|</size>|<color=.*?>|</color>|<material=.*?>|</material>|<quad name=(.+?) size=(\d*\.?\d+%?) width=(\d*\.?\d+%?) />|<a href=([^>\n\s]+)>|</a>|\s", RegexOptions.Singleline);
			...
			for (int i = 0; i < matches.Count; i++) {
				...
				if (img.sprite == null || img.sprite.name != spriteName) {
					if (iconList != null && iconList.Count > 0) {
						for (int s = 0; s < iconList.Count; s++) {
							if (iconList[s].name == spriteName) {
								img.sprite = iconList[s].sprite;
								img.preserveAspect = true;
								img.rectTransform.sizeDelta = new Vector2(fontSize * ImageScalingFactor * iconList[s].scale.x,
																			fontSize * ImageScalingFactor * iconList[s].scale.y);
								imgoffset = iconList[s].offset;
	...
	protected string GetOutputText() {
		...
		if (iconList != null && iconList.Count > 0) {
			for (int i = 0; i < iconList.Count; i++) {
				if (!string.IsNullOrEmpty(iconList[i].name)) {
					fixedString = fixedString.Replace(iconList[i].name,
													"<quad name=" + iconList[i].name + " size=" + fontSize + " width=1 />");
				}
			}
		}
	...
	protected override void OnPopulateMesh(VertexHelper toFill) {
		...
		for (int i = 0; i < m_ImagesVertexIndex.Count; i++) {
			...
			//StartCoroutine(ClearUpPunctuationMode(this)); // 避首尾标点添加的
		}
